...
type Config struct {
	...
	Type      string `json:"type"`       // 支付类型，如 USDT-TRC20
...
// NewClient 创建客户端
func NewClient(baseURL, secretKey, paymentType string) *Client {
	return &Client{
		config: Config{
			BaseURL:   baseURL,
			SecretKey: secretKey,
...
// SetNotifyURL 设置回调地址
func (c *Client) SetNotifyURL(notifyURL string) {
	c.config.NotifyURL = notifyURL
}
...
type Order struct {
	OrderNo     string  // 订单号
	Amount      float64 // 金额(USDT)
	RedirectURL string  // 支付完成后的跳转地址
}

// CreateOrder 创建支付订单
func (c *Client) CreateOrder(order Order) (*CreateOrderResponse, error) {
